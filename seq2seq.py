...
class CategoricalEmbedding(nn.Module):
    ...
    def __init__(self, cat_emb_para_l):
        ...
        '''
        super().__init__()
        self.cat_emb_layer_l = nn.ModuleList([nn.Embedding(cat_emb_para[1], cat_emb_para[2])
...
class FixedFeatEmbedding(nn.Module):
    ...
    def __init__(self, fixed_emb_para_l):
        ...
        self.fixed_emb_layer_l = nn.ModuleList([nn.Embedding(fixed_emb_para[1], fixed_emb_para[2])
...
class ContextEnhanceLayer(nn.Module):
    def __init__(self, context_in_dim, context_out_dim):
        ...
        self.classifier = nn.Sequential(
            nn.Linear(context_in_dim, context_out_dim, bias=False),
            nn.Tanh()
        )
...
class Conv1DLayer(nn.Module):
    ...
    def __init__(self, out_channels, kernel_size):
        ...
        self.conv1d = nn.Conv1d(1, out_channels, kernel_size, padding=int((kernel_size-1)/2), padding_mode='reflect')
...
class Seq2Seq(nn.Module):
    def __init__(self,
        ...
        self.cat_emb_layer = CategoricalEmbedding(categorical_feat_emb_lookup).to(device)
        ...
        self.context_layer = ContextEnhanceLayer(hid_dim_encode, hid_dim_encode).to(device)
        self.seq_conv_layer = Conv1DLayer(conv1d_output_dim, conv1d_kernal_size).to(device)
        ...
        input_dim_encode = (1 +
                            ...
                            len(xdaysago) +
                            conv1d_output_dim)
        self.encoder = EncoderRNN(input_dim_encode, hid_dim_encode, n_layers_rnn, dropout_rnn).to(device)
        ...
        input_dim_decode = (1 +
                            ...
                            sum([fixed_feat[2] for fixed_feat in fixed_feat_emb_lookup]) +
                            len(xdaysago))
        self.decoder = DecoderRNN(input_dim_decode, hid_dim_decode, n_layers_rnn, dropout_rnn).to(device)
    ...
    def forward(self, src_ts, trg_ts, src_xdaysago, trg_xdaysago, cat_encode, cat_decode, fixed_feat, teacher_forcing_ratio = 0.5):
        '''
        ...
        encode_input = torch.cat([src_ts, conv1d_emb, src_xdaysago, cat_encode_emb, fixed_emb], 2)

        # we don't use conv1d feature as decoder initial input
        encode_input_for_first_decode = torch.cat([src_ts, src_xdaysago, cat_encode_emb, fixed_emb], 2)
        ...
        for t in range(decode_len):
            ...
            if teacher_force:
                decode_input = torch.cat([trg_ts[t], trg_xdaysago[t], cat_decode_emb[t], fixed_emb[t]], 1)
            else:
                decode_input = torch.cat([final_output, trg_xdaysago[t], cat_decode_emb[t], fixed_emb[t]], 1)
